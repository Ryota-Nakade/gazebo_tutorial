...
* omni_controller.yaml
* omni_joint_state_controller.yaml
...
* omni_gazebo_twist.launch  
 : オムニホイールのシミュレーションをgazebo上で実行する．
 twistを使ってコントロールできる．

* omni_gazebo.launch  
    : オムニホイールのシミュレーションをgazebo上で実行するファイル．
    ```
    $ roslaunch gazbo_tutorial omni_gazebo.launch 
    ```
    以下のコマンドでホイールを回転させられる．
    ```
